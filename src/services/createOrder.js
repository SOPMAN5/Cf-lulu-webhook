...
const handler = {};
const BASE_URL = "https://api.lulu.com";
...
handler.createPrintJobs = async (order) => {
	...
	const url = "https://api.lulu.com" + "/" + "print-jobs/";

	const order_detail = await buildPrintJobs(order);
	console.log(order_detail, 'dd')
	try {
		if (!order_detail) {
			return {
				status: false,
		...
		const result = await customAxios("application/json").post(
			url,
			order_detail
		);
